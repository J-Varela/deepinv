...
| [`deepinv.optim.RDP`](https://deepinv.org/api/stubs/deepinv.optim.RDP.html.md#deepinv.optim.RDP)                   | $\reg{x}=\sum_{\{j,k\}\in\mathcal{N}}\frac{(x_j-x_k)^2}{x_j+x_k+\gamma|x_j-x_k|}$ | Yes                   |
...
| [`deepinv.optim.BSREM`](https://deepinv.org/api/stubs/deepinv.optim.BSREM.html.md#deepinv.optim.BSREM)     | $x_{k,l+1} = \mathcal{P}_{+}\left[x_{k,l} - \alpha_k \frac{x_{k,l}}{p} \odot \left(\nabla f_l(x_{k,l}) + \frac{\lambda}{L}\nabla \reg{x_{k,l}}\right)\right]$<br/><br/>                                                      |
| [`deepinv.optim.BlindRL`](https://deepinv.org/api/stubs/deepinv.optim.BlindRL.html.md#deepinv.optim.BlindRL) | $h_{k+1} = \Pi_{\Delta}\left(\frac{h_k}{x_k^\dagger * 1} \odot x_k^\dagger * \frac{y}{x_k * h_k}\right)$<br/><br/><br/>$x_{k+1} = \frac{x_k}{h_{k+1}^\dagger * 1} \odot h_{k+1}^\dagger * \frac{y}{h_{k+1} * x_k}$<br/><br/> |

See also the [blind inverse problems guide](https://deepinv.org/user_guide/reconstruction/blind.html.md#blind) for algorithms that jointly estimate the reconstruction and unknown physics parameters.
