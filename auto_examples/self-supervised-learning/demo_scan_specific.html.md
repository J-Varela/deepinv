...
```none
/local/jtachell/deepinv/deepinv/deepinv/datasets/fastmri.py:357: DeprecationWarning: The tuple format for dataset outputs is deprecated and will be removed in a future version.It is recommended to set `use_dict_output=True` for better readability and flexibility in returned outputs.The default is currently `False` for backward compatibility, but will be switched to `True` in a future version.
...
<!-- GENERATED FROM PYTHON SOURCE LINES 184-224 -->
```Python
...
**Total running time of the script:** (1 minutes 25.266 seconds)
