...
<a id="sphx-glr-auto-examples-self-supervised-learning-demo-n2n-denoising-py"></a>
...
The Neighbor2Neighbor loss is presented in Huang *et al.*<sup>[1](#footcite-huang2021neighbor2neighbor)</sup> and is defined as:

$$
\| A_2 y - R(A_1 y)\|^2 + \gamma \| A_2 y - R(A_1 y) - (A_2 R(y) - A_1 R(y))\|^2
$$

where $A_1$ and $A_2$ are two masks, each choosing a different neighboring map,
$R$ is the trainable denoiser network, $\gamma>0$ is a regularization parameter
...
We set [`deepinv.loss.Neighbor2Neighbor`](https://deepinv.org/api/stubs/deepinv.loss.Neighbor2Neighbor.html.md#deepinv.loss.Neighbor2Neighbor) as the training loss.
...
<!-- GENERATED FROM PYTHON SOURCE LINES 159-193 -->
```Python
...
```none
Eval epoch 0: TotalLoss=0.067, PSNR=23.97, PSNR no learning=19.487
Test results:
PSNR no learning: 19.487 +- 1.628
PSNR: 23.970 +- 1.144

{'PSNR no learning': 19.487266845703125, 'PSNR no learning_std': 1.6277921401540443, 'PSNR': 23.969549102783205, 'PSNR_std': 1.1435057751067021, 'runtime': 0.34845476085320115, 'peak_gpu_memory_usage': 0.9244284629821777}
```
...
<a id="sphx-glr-download-auto-examples-self-supervised-learning-demo-n2n-denoising-py"></a>
