...
```none
Selected GPU 0 with 8198.4375 MiB free memory
```
...
Finally, we use [`deepinv.physics.TomographyWithAstra`](https://deepinv.org/api/stubs/deepinv.physics.TomographyWithAstra.html.md#deepinv.physics.TomographyWithAstra) to instantiate the forward/backward projectors:

#### TIP
You can also use [`deepinv.datasets.DeteCTDataset.get_astra_geometry()`](https://deepinv.org/api/stubs/deepinv.datasets.DeteCTDataset.html.md#deepinv.datasets.DeteCTDataset.get_astra_geometry) to get `obj_geom, proj_geom` directly, passing `problem='sparse_view', n_angles=n_angles`.
...
```none
Power iteration converged at iteration 10, ||A^T A||_2=253497.38
```
...
The `A_dagger` method of [`deepinv.physics.TomographyWithAstra`](https://deepinv.org/api/stubs/deepinv.physics.TomographyWithAstra.html.md#deepinv.physics.TomographyWithAstra) uses an approximate pseudo-inverse when `fbp=True`.
...
The [`deepinv.datasets.DeteCTDataset`](https://deepinv.org/api/stubs/deepinv.datasets.DeteCTDataset.html.md#deepinv.datasets.DeteCTDataset) class can be used to load `x, y`, where `y` are the already-preprocessed sinograms as above,
...
```none
tensor(0.0187, device='cuda:0') tensor(0.0173, device='cuda:0') tensor(0.0200, device='cuda:0')
```
...
For the full benchmark, use [`deepinv.datasets.DeteCTDataset`](https://deepinv.org/api/stubs/deepinv.datasets.DeteCTDataset.html.md#deepinv.datasets.DeteCTDataset) and process them using [`deepinv.Trainer.test()`](https://deepinv.org/api/stubs/deepinv.Trainer.html.md#deepinv.Trainer.test).
...
#### NOTE
[`deepinv.Trainer.test()`](https://deepinv.org/api/stubs/deepinv.Trainer.html.md#deepinv.Trainer.test) does not do any manual rescaling, so we use `min_max` rescale mode for plotting. Therefore, “no learning recon” and RAM appear with different visual intensities.
...
Test:   0%|          | 0/1 [00:02<?, ?it/s, PSNR=39.2, PSNR no learning=34.6]
Test: 100%|██████████| 1/1 [00:03<00:00,  3.95s/it, PSNR=39.2, PSNR no learning=34.6]
Test: 100%|██████████| 1/1 [00:03<00:00,  3.95s/it, PSNR=39.2, PSNR no learning=34.6]
Test results:
PSNR no learning: 34.606 +- 0.007
PSNR: 39.192 +- 0.000

{'PSNR no learning': 34.6063232421875, 'PSNR no learning_std': 0.006921505128093182, 'PSNR': 39.192466735839844, 'PSNR_std': 0.0, 'runtime': 3.9539700327441096, 'peak_gpu_memory_usage': 3.819027900695801}
```
...
<!-- GENERATED FROM PYTHON SOURCE LINES 271-319 -->
```Python
n_angles = 1200
proj_geom = astra.create_proj_geom(
...
```none
Power iteration converged at iteration 17, ||A^T A||_2=869455.25
```

<!-- GENERATED FROM PYTHON SOURCE LINES 320-322 -->

Note that, similar above, you can also use [`deepinv.Trainer.test()`](https://deepinv.org/api/stubs/deepinv.Trainer.html.md#deepinv.Trainer.test) to test the model on the full test dataset.
...
<!-- GENERATED FROM PYTHON SOURCE LINES 335-393 -->
```Python
proj_geom = astra.create_proj_geom(
...
```none
Power iteration converged at iteration 10, ||A^T A||_2=2534973.25
```

<!-- GENERATED FROM PYTHON SOURCE LINES 394-396 -->

Similarly you can also use [`deepinv.Trainer.test()`](https://deepinv.org/api/stubs/deepinv.Trainer.html.md#deepinv.Trainer.test) to test the model on the full low-dose test dataset.
The results on the test set should then be comparable to those reported in the benchmark in Kiss *et al.*<sup>[1](#footcite-kiss2025benchmarking)</sup>.
...
**Total running time of the script:** (0 minutes 45.570 seconds)
