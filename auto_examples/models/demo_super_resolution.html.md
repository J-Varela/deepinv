...
<!-- "auto_examples/models/demo_super_resolution.py" -->
<!-- LINE NUMBERS ARE GIVEN BELOW. -->

<a id="sphx-glr-auto-examples-models-demo-super-resolution-py"></a>

# Super-resolution with SRResNet

Single-image super-resolution (SISR) is the inverse problem of recovering a
high-resolution (HR) image $x$ from a low-resolution (LR) observation
$y = \downarrow_s(x)$, where $\downarrow_s$ denotes downsampling
by factor $s$.
...
[`SRResNet`](https://deepinv.org/api/stubs/deepinv.models.SRResNet.html.md#deepinv.models.SRResNet) <sup>[1](#footcite-ledig2017photo)</sup> is a *direct* feed-forward
...
2. **Fine-tuning** with [`deepinv.Trainer`](https://deepinv.org/api/stubs/deepinv.Trainer.html.md#deepinv.Trainer) to show the model is fully trainable.
...
```none
Selected GPU 0 with 21594.4375 MiB free memory
```
...
the L1 loss, using [`DownsamplingMatlab`](https://deepinv.org/api/stubs/deepinv.physics.DownsamplingMatlab.html.md#deepinv.physics.DownsamplingMatlab) (MATLAB-style
...
We apply [`DownsamplingMatlab`](https://deepinv.org/api/stubs/deepinv.physics.DownsamplingMatlab.html.md#deepinv.physics.DownsamplingMatlab) to match the training
...
SRResNet is fully trainable with [`Trainer`](https://deepinv.org/api/stubs/deepinv.Trainer.html.md#deepinv.Trainer). To demonstrate
this, we fine-tune the pretrained model on a small subset of Urban100.

<!-- GENERATED FROM PYTHON SOURCE LINES 121-154 -->
```Python
...
```none
/local/jtachell/deepinv/deepinv/deepinv/datasets/datagenerator.py:612: UserWarning: Dataset ./dinv_dataset0.h5 already exists, this will close and overwrite the previous dataset.
  warn(
...
Train epoch 0: TotalLoss=0.054, PSNR=22.485
Eval epoch 0: PSNR=16.805
Best model saved at epoch 1
Train epoch 1: TotalLoss=0.051, PSNR=22.961
Eval epoch 1: PSNR=16.626
Train epoch 2: TotalLoss=0.049, PSNR=23.46
Eval epoch 2: PSNR=16.59
Train epoch 3: TotalLoss=0.048, PSNR=23.739
Eval epoch 3: PSNR=16.525
Train epoch 4: TotalLoss=0.047, PSNR=23.789
Eval epoch 4: PSNR=16.361
Train epoch 5: TotalLoss=0.046, PSNR=24.181
Eval epoch 5: PSNR=16.387
Train epoch 6: TotalLoss=0.045, PSNR=24.359
Eval epoch 6: PSNR=16.313
Train epoch 7: TotalLoss=0.044, PSNR=24.553
Eval epoch 7: PSNR=16.225
Train epoch 8: TotalLoss=0.043, PSNR=24.709
Eval epoch 8: PSNR=16.272
Train epoch 9: TotalLoss=0.042, PSNR=24.774
Eval epoch 9: PSNR=16.296
Model, optimizer, epoch_start successfully loaded from checkpoint: 26-09-21-09:58:58/ckp_best.pth.tar
```
...
<a id="sphx-glr-download-auto-examples-models-demo-super-resolution-py"></a>
