...
<a id="sphx-glr-auto-examples-blind-inverse-problems-demo-blind-deblurring-py"></a>
...
the paper Carbajal *et al.*<sup>[1](#footcite-carbajal2023blind)</sup>. The network estimates spatially-varying blur kernels from a blurred image,
...
where $\star$ is a convolution with circular padding, $\odot$ is a Hadamard product, $w_k$ are masks
and $h_k$ are filters.
...
```none
Selected GPU 0 with 18054.4375 MiB free memory
```
...
You can access the whole dataset using [`deepinv.datasets.Kohler`](https://deepinv.org/api/stubs/deepinv.datasets.Kohler.html.md#deepinv.datasets.Kohler).
...
Here we use the general reconstruction model [`deepinv.models.RAM`](https://deepinv.org/api/stubs/deepinv.models.RAM.html.md#deepinv.models.RAM) and the plug-and-play method [`deepinv.optim.DPIR`](https://deepinv.org/api/stubs/deepinv.optim.DPIR.html.md#deepinv.optim.DPIR).
...
**Total running time of the script:** (1 minutes 6.355 seconds)

🏷 Tags: Deblurring, Blind problem, Plug-and-play, Foundation model

<a id="sphx-glr-download-auto-examples-blind-inverse-problems-demo-blind-deblurring-py"></a>
