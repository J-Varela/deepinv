...
<a id="sphx-glr-auto-examples-physics-demo-microscopy-3d-py"></a>
...
```none
Selected GPU 0 with 20100.4375 MiB free memory
```
...
The class [`deepinv.physics.Blur`](https://deepinv.org/api/stubs/deepinv.physics.Blur.html.md#deepinv.physics.Blur) implements convolution operations with kernels. It will
automatically work for 3D images when given 5D [`torch.Tensor`](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor) of size (B, C, D, H, W) for both
...
```none
/local/jtachell/deepinv/deepinv/deepinv/utils/plotting.py:1292: UserWarning: This figure was using a layout engine that is incompatible with subplots_adjust and/or tight_layout; not calling subplots_adjust.
...
the class [`deepinv.physics.generator.PSFGenerator`](https://deepinv.org/api/stubs/deepinv.physics.generator.PSFGenerator.html.md#deepinv.physics.generator.PSFGenerator), that we used to construct
...
[`deepinv.physics.generator.DiffractionBlurGenerator3D`](https://deepinv.org/api/stubs/deepinv.physics.generator.DiffractionBlurGenerator3D.html.md#deepinv.physics.generator.DiffractionBlurGenerator3D).
...
nomenclature [https://en.wikipedia.org/wiki/Zernike_polynomials#Noll’s_sequential_indices](https://en.wikipedia.org/wiki/Zernike_polynomials#Noll's_sequential_indices)
...
We implemented the PSF of a confocal microscope in [`deepinv.physics.generator.ConfocalBlurGenerator3D`](https://deepinv.org/api/stubs/deepinv.physics.generator.ConfocalBlurGenerator3D.html.md#deepinv.physics.generator.ConfocalBlurGenerator3D).
...
See Gu and Sheppard<sup>[1](#footcite-gu1991three)</sup>.

Here is a working example of a confocal microscope PSF imaging at three wavelengths.

<!-- GENERATED FROM PYTHON SOURCE LINES 215-255 -->
```Python
...
```none
/local/jtachell/deepinv/deepinv/deepinv/utils/decorators.py:53: DeprecationWarning: Argument 'num_channels' is deprecated and will be removed in a future version.
...
* <a id='footcite-gu1991three'>**[1]**</a> Min Gu and CJR Sheppard. Three-dimensional imaging in confocal fluorescent microscopy with annular lenses. *Journal of Modern Optics*, 38(11):2247–2263, 1991.

**Total running time of the script:** (0 minutes 6.266 seconds)

🏷 Tags: Microscopy, Deblurring

<a id="sphx-glr-download-auto-examples-physics-demo-microscopy-3d-py"></a>
