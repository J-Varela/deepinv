...
<!-- "auto_examples/physics/demo_scattering.py" -->
<!-- LINE NUMBERS ARE GIVEN BELOW. -->

<a id="sphx-glr-auto-examples-physics-demo-scattering-py"></a>

# Inverse scattering problem

In this example we show how to use the [`deepinv.physics.Scattering`](https://deepinv.org/api/stubs/deepinv.physics.Scattering.html.md#deepinv.physics.Scattering) forward model.
...
For each of the $i=1,\dots,T$ transmitters, the 2D forward model is given by the inhomogeneous Helmholtz equation (see e.g., Soubies *et al.* [[154](https://deepinv.org/user_guide/other/biblio.html.md#id19)]):
...
and $G_s$ denotes the convolution with Green’s operator and sampling at the $R$ different receiver locations.
...
where $\lambda$ is the wavelength of the incident wave and $L$ is the length of the box where the object is located (i.e., the square of length 1 in the example below).
...
```none
Selected GPU 0 with 21980.4375 MiB free memory
```
...
where $G$ is the convolution operator with Green’s function $g$, and $b_i = G (x \circ v_i)$.

This linear system becomes highly ill-conditioned for high contrast objects (i.e., large $\|x\|_{\infty}$) and/or high wavenumber
(which induces a high spectral norm of the Green operator $\|G\|_2$), and the solver
may fail to converge. In that case, one can try to increase the number of iterations, or change the solver (see
[`deepinv.physics.Scattering.SolverConfig`](https://deepinv.org/api/stubs/deepinv.physics.Scattering.html.md#deepinv.physics.Scattering.SolverConfig) for more details).
...
```none
/local/jtachell/deepinv/deepinv/deepinv/physics/scattering.py:31: DeprecationWarning: __array_wrap__ must accept context and return_scalar arguments (positionally) in the future. (Deprecated NumPy 2.0)
  return jv(n, x.detach().cpu()).to(device=device, dtype=dtype)
/local/jtachell/deepinv/deepinv/deepinv/physics/scattering.py:18: DeprecationWarning: __array_wrap__ must accept context and return_scalar arguments (positionally) in the future. (Deprecated NumPy 2.0)
  return hankel1(n, x.detach().cpu()).to(device=device, dtype=dtype)
LSQR converged at iteration 23
```
...
gradient evaluation (see e.g. Soubies *et al.* [[154](https://deepinv.org/user_guide/other/biblio.html.md#id19)]), and does not require storing all intermediate variables (as in standard backpropagation via PyTorch autograd).
...
<!-- GENERATED FROM PYTHON SOURCE LINES 187-233 -->
```Python
...
```none
LSQR converged at iteration 22
LSQR converged at iteration 21
Peak GPU memory usage for grad computation: 777.0 MB
Peak GPU memory usage for grad computation: 901.2 MB
Difference between gradients: 1.0817071682822643e-09
```
...
```none
PSNR of Born approximation: 14.543635 dB
```
...
```none
PSNR of gradient descent reconstruction: 23.68 dB
```
...
<!-- GENERATED FROM PYTHON SOURCE LINES 315-342 -->
```Python
if device != "cpu":
...
- Use a pretrained denoiser to perform plug-and-play reconstruction, as in [Vanilla PnP for computed tomography (CT).](https://deepinv.org/auto_examples/plug-and-play/demo_vanilla_PnP.html.md#sphx-glr-auto-examples-plug-and-play-demo-vanilla-pnp-py)
- Learn a reconstruction network using an unrolled architecture, as in [Vanilla Unfolded algorithm for super-resolution](https://deepinv.org/auto_examples/unfolded/demo_vanilla_unfolded.html.md#sphx-glr-auto-examples-unfolded-demo-vanilla-unfolded-py).

<!-- GENERATED FROM PYTHON SOURCE LINES 354-357 -->
* **References:**

**Total running time of the script:** (1 minutes 8.815 seconds)

🏷 Tags: Tomography

<a id="sphx-glr-download-auto-examples-physics-demo-scattering-py"></a>
