...
<!-- "auto_examples/physics/demo_pet2d.py" -->
<!-- LINE NUMBERS ARE GIVEN BELOW. -->

<a id="sphx-glr-auto-examples-physics-demo-pet2d-py"></a>
...
Here we define each pixel to have size $3\times 3$ mm
such that the total area to reconstruct is of size $38.4\times 38.4$ cm
...
carefully handle the normalization. See also the [unnormalized 3D PET example](https://deepinv.org/auto_examples/physics/demo_pet3d.html.md#sphx-glr-auto-examples-physics-demo-pet3d-py).
...
The sensitivity map is defined as the back-projection of a sinogram of ones $s = A^\top \mathbf{1}$,
...
/local/jtachell/deepinv/deepinv/deepinv/physics/forward.py:639: DeprecationWarning: Using `compute_norm(squared=True)` is deprecated. Use `compute_sqnorm()` instead to compute the squared spectral norm (||A^T A||_2). In a future version, `compute_norm()` will compute the non-squared spectral norm (||A||_2) by default.
  warnings.warn(
Power iteration converged at iteration 9, ||A^T A||_2=0.93
Norm operator: 0.93
```
...
<!-- GENERATED FROM PYTHON SOURCE LINES 224-287 -->
```Python
...
```none
MLEM runtime: 0.43 s for 40 iterations
...
Alternatively, pre-split inputs can be created with  [`deepinv.physics.split_measurements()`](https://deepinv.org/api/stubs/deepinv.physics.split_measurements.html.md#deepinv.physics.split_measurements)
and [`deepinv.physics.split_physics()`](https://deepinv.org/api/stubs/deepinv.physics.split_physics.html.md#deepinv.physics.split_physics) and passed directly to [`deepinv.optim.OSEM`](https://deepinv.org/api/stubs/deepinv.optim.OSEM.html.md#deepinv.optim.OSEM).

<!-- GENERATED FROM PYTHON SOURCE LINES 315-386 -->
```Python
osem_iter = 5
num_subsets = 8
...
```none
OSEM runtime: 0.84 s for 5 iterations (8 subsets)
Reconstruction speedup MLEM/OSEM: 0.51x
/local/jtachell/deepinv/deepinv/deepinv/utils/plotting.py:493: UserWarning: set_ticklabels() should only be used with a fixed number of ticks, i.e. after set_ticks() or using a FixedLocator. Otherwise, ticks may be mislabeled.
  colbar.set_ticklabels(true_labels)
/local/jtachell/deepinv/deepinv/deepinv/utils/plotting.py:493: UserWarning: set_ticklabels() should only be used with a fixed number of ticks, i.e. after set_ticks() or using a FixedLocator. Otherwise, ticks may be mislabeled.
  colbar.set_ticklabels(true_labels)
...
- Check out the [3D PET example](https://deepinv.org/auto_examples/physics/demo_pet3d.html.md#sphx-glr-auto-examples-physics-demo-pet3d-py).
...
<a id="sphx-glr-download-auto-examples-physics-demo-pet2d-py"></a>
