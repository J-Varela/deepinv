...
<!-- "auto_examples/distributed/demo_pnp_distributed.py" -->
<!-- LINE NUMBERS ARE GIVEN BELOW. -->

<a id="sphx-glr-auto-examples-distributed-demo-pnp-distributed-py"></a>
...
[`deepinv.distributed.distribute()`](https://deepinv.org/api/stubs/deepinv.distributed.distribute.html.md#deepinv.distributed.distribute).
...
- $\ell_2$ data fidelity gradient computed using [`deepinv.distributed.framework.DistributedDataFidelity.grad()`](https://deepinv.org/api/stubs/deepinv.distributed.framework.DistributedDataFidelity.html.md#deepinv.distributed.framework.DistributedDataFidelity.grad)

**Key Steps:**

1. Create stacked physics operators and measurements with reproducible noise
2. Initialize distributed context
3. Distribute physics with [`deepinv.distributed.distribute()`](https://deepinv.org/api/stubs/deepinv.distributed.distribute.html.md#deepinv.distributed.distribute)
4. Distribute denoiser with tiling configuration
5. Create PnP prior and $\ell_2$ data fidelity
6. Run PnP iterations using [`deepinv.distributed.framework.DistributedDataFidelity.grad()`](https://deepinv.org/api/stubs/deepinv.distributed.framework.DistributedDataFidelity.html.md#deepinv.distributed.framework.DistributedDataFidelity.grad) for gradient computation
...
<!-- GENERATED FROM PYTHON SOURCE LINES 60-131 -->
```Python
import torch
...
<!-- GENERATED FROM PYTHON SOURCE LINES 148-322 -->
```Python
...
Running on 1 process(es)
   Device: cuda:0
/local/jtachell/deepinv/deepinv/examples/distributed/demo_pnp_distributed.py:98: DeprecationWarning: Function 'gaussian_blur' is deprecated since 0.4.1. Use 'deepinv.physics.functional.blur.gaussian_blur' instead.
  gaussian_blur(sigma=1.0, device=str(device)),  # Small blur
/local/jtachell/deepinv/deepinv/examples/distributed/demo_pnp_distributed.py:99: DeprecationWarning: Function 'gaussian_blur' is deprecated since 0.4.1. Use 'deepinv.physics.functional.blur.gaussian_blur' instead.
  gaussian_blur(sigma=2.0, device=str(device)),  # Medium blur
/local/jtachell/deepinv/deepinv/examples/distributed/demo_pnp_distributed.py:100: DeprecationWarning: Function 'gaussian_blur' is deprecated since 0.4.1. Use 'deepinv.physics.functional.blur.gaussian_blur' instead.
  gaussian_blur(
...
Running PnP reconstruction (20 iterations)...
   ...
   Iteration 15/20, PSNR: 32.57 dB
   Iteration 20/20, PSNR: 32.70 dB

Comparing with non-distributed PnP reconstruction...
   Non-distributed final PSNR: 32.68 dB
   Distributed final PSNR:     32.70 dB
   PSNR difference:             0.02 dB
   Mean absolute difference:    9.69e-04
   Max absolute difference:     1.64e-01
   Results match well!
...
🏷 Tags: Distributed, Plug-and-play

<a id="sphx-glr-download-auto-examples-distributed-demo-pnp-distributed-py"></a>
