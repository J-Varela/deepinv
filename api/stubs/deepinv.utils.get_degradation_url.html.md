# get_degradation_url

### deepinv.utils.get_degradation_url(file_name)

Get URL for degradation from DeepInverse HuggingFace repository.

* **Parameters:**
  **file_name** ([*str*](https://docs.python.org/3.9/library/stdtypes.html#str)) – degradation filename in repository
* **Returns:**
  degradation URL
* **Return type:**
  [str](https://docs.python.org/3.9/library/stdtypes.html#str)

## Examples using `get_degradation_url`:

<div class="sphx-glr-thumbnails">
<!-- thumbnail-parent-div-open --><div class="sphx-glr-thumbcontainer" tooltip="In this example, we investigate a simple 2D Radio Interferometry (RI) imaging task with deepinverse. The following example and data are taken from aghabiglou2024r2d2. If you are interested in RI imaging problem and would like to see more examples or try the state-of-the-art algorithms, please check BASPLib.">  <div class="sphx-glr-thumbnail-title">Radio interferometric imaging with deepinverse</div>
</div>
<!-- thumbnail-parent-div-close --></div>
