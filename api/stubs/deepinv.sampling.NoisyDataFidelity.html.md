...
### *class* deepinv.sampling.NoisyDataFidelity(weight=1.0, \*args, \*\*kwargs)

Bases: [`DataFidelity`](https://deepinv.org/api/stubs/deepinv.optim.DataFidelity.html.md#deepinv.optim.DataFidelity)

Data fidelity term for noisy input data $- \log p(y|x + \sigma(t) \omega)$ with $\omega\sim\mathcal{N}(0,\mathrm{I})$.

This is a base class used for approximating $- \log p_t(y|x_t)$ used in diffusion
...
It comes with a `.grad` method computing the negative log-likelihood gradient :math:\` - nabla_{x_t} log p_t(y|x_t)\`.
...
#### NOTE
Unlike [`deepinv.optim.DataFidelity`](https://deepinv.org/api/stubs/deepinv.optim.DataFidelity.html.md#deepinv.optim.DataFidelity), these terms are defined through their
gradient only: the approximations of $- \log p_t(y|x_t)$ used by diffusion
...
$-\nabla_{x_t} \log p_t(y|x_t)$, see [`grad()`](#deepinv.sampling.NoisyDataFidelity.grad). Subclasses whose
approximation of $- \log p_t(y|x_t)$ has a closed form override this method.

#### grad(x, y, physics, \*args, \*\*kwargs)

Computes the gradient of the data-fidelity term $\lambda A^\top(A(x) - y)$.
